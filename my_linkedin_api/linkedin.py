...
from my_linkedin_api.client import Client
from my_linkedin_api.utils.helpers import (
    get_id_from_urn,
...
class Linkedin(object):
    ...
    def get_geo_id(self, location_name: str) -> Optional[str]:
        """
        Fetch the geoId for a given location name using LinkedIn's internal GraphQL API.

        :param location_name: The name of the location (e.g., "US" for United States, or "Toronto")
        :type location_name: str
        :return: The geoId as a string, or None if not found
        :rtype: Optional[str]
        """
        # GraphQL endpoint format with proper LinkedIn query syntax
        variables = (
            f"(keywords:{location_name},"
            "query:(typeaheadFilterQuery:(geoSearchTypes:List(POSTCODE_1,POSTCODE_2,POPULATED_PLACE,"
            "ADMIN_DIVISION_1,ADMIN_DIVISION_2,COUNTRY_REGION,MARKET_AREA,COUNTRY_CLUSTER)),"
            "typeaheadUseCase:JOBS),"
            "type:GEO)"
        )

        query_id = "voyagerSearchDashReusableTypeahead.35c83322e303eeb7ced9eb48e83a165c"
        ...
        if res.status_code != 200:
            self.logger.error(f"Failed to get geoId for {location_name}")
            return None

        try:
            # results = res.json().get("data", {}).get("typeahead", {}).get("results", [])
            # print(results)
            # for result in results:
            #     tracking_urn = result.get("trackingUrn")
            #     if tracking_urn and tracking_urn.startswith("urn:li:geo:"):
            #         return tracking_urn.split(":")[-1]  # Extract the geoId
            ...
            for element in elements:
                title = element.get("trackingUrn", "")
                if title and title.startswith("urn:li:geo:"):
                    return title.split(":")[-1]

                title = element.get("title", {}).get("text", "")
                if title.lower() == location_name.lower():
                    geo_urn = element.get("target", {}).get("*geo", "")
                    if geo_urn.startswith("urn:li:fsd_geo:"):
                        return geo_urn.split(":")[-1]
                
        except Exception as e:
            self.logger.error(f"Error parsing geoId response: {str(e)}")
    ...
    def search_jobs(
        ...
        **kwargs,
        ...
        query["selectedFilters"]["sortBy"] = "List(R)"  # relevance
